...
type Resource = {
  title: string; author: string; authorBadge: string; type: string; category: string;
  rating: number; reviews: number; img: string; description: string; tags: string[]; recommended: boolean;
}
...
export default function ResourcesView() {
  ...
  const filtered = RESOURCES.filter(r => {
    const matchType = type === 'All' || r.type === type || (type === 'Books' && r.type === 'Book')
            ...
            <div key={i} style={{
              ...
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <img
                  ...
                  style={{ width: '84px', height: '112px', objectFit: 'cover', borderRadius: '8px', display: 'block', boxShadow: '0 4px 14px rgba(0,0,0,0.18)' }}
                />
                {r.recommended && (
                  <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                  ...
                  <span style={{ fontSize: '12px', color: 'var(--color-text-2)' }}>({(r.reviews ?? 0).toLocaleString()} reviews)</span>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
                  ...
                  <button
                    onClick={() => setSaved(prev => {
                      ...
                      if (isSaved) next.delete(i); else next.add(i)
                      return next
                    })}
                    style={{
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '64px 24px', backgroundColor: 'var(--color-card)', borderRadius: '14px', border: '1px solid var(--color-border)' }}>
            ...
            <div style={{ fontSize: '14px', color: 'var(--color-text-2)', lineHeight: 1.6 }}>
              {RESOURCES.length === 0 ? 'Books, courses, and teachings from FMCI leaders will appear here.' : 'Try a different category or type filter.'}
            </div>
          </div>
        )}
      </div>
